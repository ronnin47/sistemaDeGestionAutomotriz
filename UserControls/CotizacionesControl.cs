...
namespace sistemaDeGestionAutomotriz.UserControls
{
    ...
    {
        private readonly CotizacionService _service = new CotizacionService();
        private List<CotizacionPendienteDto> _cotizaciones = new List<CotizacionPendienteDto>();
        ...
        private void ConstruirUI()
        {
            ...
            _lblTitulo = new Label { Text = "Cotizaciones pendientes", Location = new Point(0, 0) };
            ...
            // Fill primero, Top después (el último queda arriba de todo).
        ...
        {
            _cotizaciones = _service.ObtenerClientesPendientes() ?? new List<CotizacionPendienteDto>();
            AplicarFiltro();
        }
        ...
        private void AplicarFiltro()
        {
            ...
            List<CotizacionPendienteDto> visibles = _cotizaciones;
            if (q.Length > 0)
            {
                visibles = _cotizaciones.Where(c =>
                    ((c.Cliente ?? "") + " " + (c.TipoServicio ?? "")).ToLower().Contains(q)).ToList();
            }

            _grilla.DataSource = null;
            _grilla.DataSource = visibles;

            bool sinDatos = _cotizaciones.Count == 0;
        ...
        {
            int total = _cotizaciones.Count;
            if (total == visibles)
                _lblContador.Text = total == 1 ? "1 cotización pendiente" : total + " cotizaciones pendientes";
            else
                _lblContador.Text = visibles + " de " + total + " cotizaciones";
        }
    }
}
